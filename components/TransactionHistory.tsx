...
const TransactionHistory: React.FC<TransactionHistoryProps> = ({ transactions, onUpdateStatus, title = "Order & Kitchen Pipeline" }) => {
              ...
              return (
                <tr key={tx.id} className="hover:bg-gray-50/80 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <select 
                      value={tx.status} 
                      onChange={(e) => onUpdateStatus(tx.id, e.target.value as OrderStatus)}
