...
const addItem = (itemData: Omit<MenuItem, 'id'>): Promise<MenuItem> => {
...
const updateItem = (updatedItem: MenuItem): Promise<MenuItem> => {
...
const recordTransaction = (item: MenuItem): Promise<Transaction> => {
  return new Promise(async (resolve) => {
...
const updateTransactionStatus = (id: string, status: OrderStatus): Promise<void> => {
  return new Promise(async (resolve, reject) => {
    const transactions = await getTransactions();
